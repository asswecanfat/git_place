...
def main_gui(dbase):
    ...
    while True:
        ...
        if event is '登录':
            try:
                user, user_id = __reg(values[0], values[1], dbase)
                if user is not None:
                    sg.popup(f'用户{user}登录成功!!', title='提示')
                    if user_id is not None:
                        window.close()
                        ktv_ui(dbase, user_id, user)
                    else:
                        window.close()
                        manger_ui(dbase)
                    break
...
def __reg(user, password, dbase):
    result, data = dbase.deal_sql(sql='select user_name,u_password,user_id from ktv_user')

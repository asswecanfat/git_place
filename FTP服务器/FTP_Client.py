...
class FtpClient(object):
    log = attrib(type=int, default=0)  # 登录状态记录
    con = attrib(type=int, default=0)  # 数据端口连接状态记录
    msg_dic = attrib(default=None)  # 服务器端发送的头部信息记录
    host = attrib(type=str, default='192.168.43.221', kw_only=True)  # 主机号
    ...
    command_s = attrib(init=False, default=socket.socket())  # 命令socket
    data_s = attrib(init=False, default=socket.socket())  # 数据socket
    ...
    def deal_func(self, cmd, final_data, data):  # 处理方法
        if hasattr(self, 'cmd_%s' % cmd):  # 反射到类中某个函数
    ...
    def cmd_user(self, final_data):  # user指令
        self.command_s.send(bytes(final_data, encoding='utf-8'))
        data_message = self.command_s.recv(1024).decode()
        print(data_message)

    def cmd_pass(self, final_data):  # pass指令
        self.command_s.send(bytes(final_data, encoding='utf-8'))
        data_message = self.command_s.recv(1024).decode()
        print(data_message)
    ...
    def cmd_get(self, final_data, data):
        if self.log:
            ...
            with open(r'C:\Users\10248\Desktop\%s' % file_name, 'wb') as f:  # 默认在桌面
    ...
    def cmd_dir(self, final_data):  # dir指令
        if self.con == 0:
            self.data_s.connect((self.host, self.data_port))
            self.con = 1
        print('root')
